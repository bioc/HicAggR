...
#' @param secondaryConst.var <character>: A string defining column name containing compartment information in the metadata of anchor and bait <GRanges> objects. (Default NULL)
#' @param chromSizes <data.frame>: A data.frame containing chromosomes names and lengths in base pairs.
#' @param n_background <integer> : Number of background couples to keep. (Default NULL)
#' @param indexAnchor <GRanges>: A first indexed GRanges object used as pairs anchor (must be indexed using `IndexFeatures()`).
#' @param indexBait <GRanges>: A second indexed GRanges object used as pairs bait (must be indexed using `IndexFeatures()`).
#' @param cores <integer> : Number of cores used. (Default 1)
#' @param areaFun <character or function>: A character or function that allows to extract an area from each matrix that composes the matrices list (Default "center"). Look at [GetQuantif] for more info.
#' @param operationFun <character or function>: A character or function specifying the operation applied to the selected area (Default "mean"). Look at [GetQuantif] for more info.
...
#' \item "inter_TAD": If target couples were formed using TAD information with non `NULL`
...
#' \item "inter_compartment": If `secondaryConst.var` is not `NULL` and both indexAnchor and indexBait
...
#' }
#' @param ... arguments to pass to [PrepareMtxList], inorder to treat background matrices.
#' @return returns a <list> object with the z.test output for each target couple, values for the target couples and values for the background couples.
#' 
#' @export
#'
#' @examples
...
compare_to_background = function(hicList = NULL,
  ...
  .findRandomBins = function(
    ...
    if(!is.null(genomicConstraint)){
      dist_const = c(min(targetCouples$distance),max(width(genomicConstraint)))
    }else{
    ...
    binned_constraint = BinGRanges(genomicConstraint,
                                   chromSizes,
                                   binSize = resolution)
    binned_constraint$name = paste0("random_",seq(1,length(binned_constraint)))
    binned_constraint = subsetByOverlaps(
      binned_constraint,
    ...
    message(paste0("Number of background pairs: ",length(background_pairs)))
  ...
  .interTads = function(targetCouples,
    ...
    print("inter-TADs")
    if(!is.null(genomicConstraint) && 
       !all(unique(indexAnchor$constraint)%in%seqlevels(indexAnchor)) && 
       !all(unique(indexBait$constraint)%in%seqlevels(indexBait))){
      dist_const = c(min(targetCouples$distance),max(width(genomicConstraint)))
    }else{
    ...
    diff.tad.couples = as.data.frame(mcols(bg_couples)) |>
      dplyr::filter(anchor.tad_name!=bait.tad_name) |>
      rownames()
    bg_couples = bg_couples[
      which(attr(bg_couples,"NAMES")%in%diff.tad.couples)
    ]
    print(paste0("Number of inter-TAD couples ",length(bg_couples)))
    if(!is.null(secondaryConst.var)){
      ...
      diff.compart.couples = as.data.frame(mcols(bg_couples)) |>
  ...
  if(length(bg_couples) < 500){
    ...
    bg_couples = .findRandomBins(targetCouples,
                                 ...
                                 resolution,
                                 n_background)
    print(length(bg_couples))
    bg_couples = .cleanCouples(bg_couples,matrices)
    print(length(bg_couples))
  }
  
  seqinfo(bg_couples) = Seqinfo(seqnames = chromSizes$seqnames,
  ...
  z.output = lapply(target_quantifs, 
    ...
    dplyr::mutate(adj.p = p.adjust(p = p.value,method="bonferroni",n=length(target_quantifs)))
  return(list(z.test = z.output,target_quantifs=target_quantifs,bg_quantifs=bg_quantifs))
}
