...
#' \dontrun{
#' hicLst <- ImportHiC(
#' file= "/home/robel/mount/work_sschaak/ApA_Package/DATA/HiC/Public/Control/4DNFIOTPSS3L.hic",
#' hicResolution= 5000,
...
#' }
#' 
#' bedpe_path = system.file("extdata", "postprocessed_pixels_5000.bedpe", package = "HicAggR", mustWork = TRUE)
#' loops = import_loops(file_bedpe = bedpe_path,
#' chromSizes = data.frame(
#'   c("2L","2R","3L","3R","4","X","Y"),
#'   c(23513712,25286936,28110227,32079331,1348131,23542271,3667352)),
...
import_loops <- function(
  ...
  loops_gni$anchor_constraint <- data.table::foverlaps(
    ...
    y = data.table::as.data.table(binned_constraint)|>
      data.table::setkey("seqnames","start","end"),
    by.x=c("seqnames","start","end"),
    by.y=c("seqnames","start","end"))$name
