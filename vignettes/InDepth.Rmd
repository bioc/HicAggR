...
3. When multiple ranges are in a same bin (ex: 3 ChIP-seq peaks in the same 10kb bin), associated numeric variables in metadata (`variablesName.chr_vec`) can be summarized according to the defined method (`method`), Example: Max peak score of the bin is kept in metadata column `score`.  
...
## Tips
1. If `res.num` is NULL, the function will atuomatically use the resolution of the `hicLst` attributes.
