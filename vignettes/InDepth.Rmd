...
.h5 (since version 0.99.2). The pacakage imports by default the raw counts in R. Therefore,
it is necessary to perform the balancing and observed/expected
correction steps.
```{r, eval = TRUE, message = FALSE}
...
[hicTransform](https://hicexplorer.readthedocs.io/en/latest/content/tools/hicTransform.html)
program.
...
and [Matrix](https://cran.r-project.org/web/packages/Matrix/Matrix.pdf) packages.
```{r, eval = TRUE}
